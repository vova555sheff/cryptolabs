...
public class MD5Hasher
{
    ...
    private byte[] _mByteInput;

    public string ComputeHash(string inputData)
    {
        _mByteInput = new byte[inputData.Length];
        for (var i = 0; i < inputData.Length; i++)
            _mByteInput[i] = (byte)inputData[i];
        _digest = CalculateMd5Value();

        return _digest.ToString();
    }

    private MD5Digest CalculateMd5Value()
    {
        var dg = new MD5Digest();
    ...
    {
        var temp = (448 - ((_mByteInput.Length * 8) % 512));
        ...
        var sizeMsgBuff = (uint)((_mByteInput.Length) + (pad / 8) + 8);
        var sizeMsg = (ulong)_mByteInput.Length * 8;
        var bMsg = new byte[sizeMsgBuff];

        for (var i = 0; i < _mByteInput.Length; i++)
            bMsg[i] = _mByteInput[i];

        bMsg[_mByteInput.Length] |= 0x80;
